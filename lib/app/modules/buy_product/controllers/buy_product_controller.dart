...
import 'package:latest_payplus_agent/app/modules/buy_product/views/fastBuyWidget.dart';
import 'package:latest_payplus_agent/app/modules/buy_product/views/listBuyWidget.dart';
...
class BuyProductController extends GetxController {
  ...
  placeOrder() async {
    ...
    buyData.value.due = (calculateTotalPrice() - discountAmount.value) - nowPaying.value;
    ...
    for (var item in cartList) {
      ...
      buyingItem.rate =
          item.buyingPrice! - (item.buyingPrice! * (item.discountPercent! / 100));

      buyingItem.total = buyingItem.rate! * buyingItem.quantity!;

      buyingItem.totalWithoutDisc = buyingItem.rateWithoutDisc! * buyingItem.quantity!;
